var addComment = { moveForm(a, b, c, d) {
  let e,
    f,
    g,
    h,
    ...
    }; try { for (let p = 0; p < o.elements.length; p++) if (f = o.elements[p], h = !1, 'getComputedStyle' in window ? g = window.getComputedStyle(f) : document.documentElement.currentStyle && (g = f.currentStyle), (f.offsetWidth <= 0 && f.offsetHeight <= 0 || g.visibility === 'hidden') && (h = !0), f.type !== 'hidden' && !f.disabled && !h) { f.focus(); break; } } catch (q) {} return !1;
  }
},
I(a) { return document.getElementById(a); } };
